...
impl Default for PanOrbitCamera {
    fn default() -> Self {
        PanOrbitCamera { focus: Vec3::ZERO, radius: 5.0, upside_down: false }
    }
}

pub fn handle_orbit_camera(
    ...
    for (mut pan_orbit, mut transform, projection) in query.iter_mut() {
        ...
        if rotation_move.length_squared() > 0.0 {
            any = true;
            let window_size = get_window_size(primary_window);
            let delta_x = {
...
pub fn spawn_camera(mut commands: Commands) {
    ...
    commands
        .spawn((
            Camera3dBundle {
                transform: Transform::from_xyz(0.0, 4.2, 4.2).looking_at(Vec3::ZERO, Vec3::Y),
                ..default()
            },
